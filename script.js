...
/* ---------- Горизонтальные ленты: отзывы, результаты, специалисты ----------
   Один обработчик на все ленты страницы — ищем каждую .track-wrap и вешаем
   на её кнопки прокрутку. Раньше код был написан только под отзывы, и для
   двух новых лент пришлось бы копировать его дважды.

   Сама прокрутка нативная (overflow-x), поэтому свайп и колесо работают даже
   если этот код не выполнился. Кнопки доводят ленту на «экран» карточек.

   Плавность анимируем сами, а не через scrollTo({behavior:'smooth'}): в части
   движков и при включённом «уменьшить движение» smooth молча игнорируется —
   прокрутки не происходит вовсе, и кнопки выглядят сломанными. Позицию
   считаем абсолютную: при scrollBy шаги накапливают ошибку, если предыдущая
   анимация ещё идёт. */
(function () {
  ...
  document.querySelectorAll('.track-wrap').forEach(function (wrap) {
    ...
    var prev = wrap.querySelector('.track-nav--prev');
    var next = wrap.querySelector('.track-nav--next');
    ...
    function animateTo(target) {
      ...
      // document.hidden — вкладка в фоне, кадры не идут: ставим сразу
