...
# Только смысловая часть имени. Номер приписывается на месте: блоков стилей
# бывает и два, и три — сколько получится после чистки CSS. Раньше здесь
# лежал жёсткий список из девяти готовых имён, и всё, чего в нём не было,
# молча пропускалось: CSS дорос до третьего блока, style-3 не попал в папку,
# а инструкция об этом не сказала — верстальщик вставил бы неполные стили.
BLOCK_NAMES = {
    "block-1.html": "шапка, баннер и первый экран",
    "block-2.html": "отзывы участниц",
    "block-3.html": "как похудеть, фейс-йога и тейпы",
    "block-4.html": "тарифы",
    "block-5.html": "результаты и специалисты",
    "block-6.html": "развилка, финал, помощь, футер",
}

HAND = BASE / "ДЛЯ ТИЛЬДЫ"
HAND.mkdir(exist_ok=True)
for stale in HAND.glob("*.html"):
    stale.unlink()

order, unnamed = [], []
for pos, (name, content) in enumerate(files, start=1):
    if name.startswith("style-"):
        human = f"{pos:02d} — стили, часть {name[6:-5]}"
    elif name == "script.html":
        human = f"{pos:02d} — скрипт, вставлять последним"
    elif name in BLOCK_NAMES:
        human = f"{pos:02d} — {BLOCK_NAMES[name]}"
    else:
        # Имени нет — всё равно кладём и говорим вслух. Пропустить нельзя:
        # недостающий блок замечают уже на сломанной странице.
        unnamed.append(name)
        human = f"{pos:02d} — {name[:-5]}"
    (HAND / f"{human}.html").write_text(content, encoding="utf-8")
    order.append(human)

if unnamed:
    print("ВНИМАНИЕ: блоки без понятного имени, положены как есть — "
          + ", ".join(unnamed) + " (допишите в BLOCK_NAMES)")
